...
class App:
    ...
    def init_gui(self):
        ...
        self.head_label.pack(anchor=tk.CENTER, expand=True)
        ...
        self.btn_toggleauto = tk.Button(self.window, text="Toggle Auto Prediction", width=50, font=("Helvetica", 12),
                                        command=self.auto_predict_toggle)
        self.btn_toggleauto.pack(anchor=tk.CENTER, expand=True)

        self.btn_class_one = tk.Button(self.window, text=self.classname_one, width=50, font=("Helvetica", 12),
                                       command=lambda: self.save_for_class(1))
        self.btn_class_one.pack(anchor=tk.CENTER, expand=True)

        self.btn_class_two = tk.Button(self.window, text=self.classname_two, width=50, font=("Helvetica", 12),
                                       command=lambda: self.save_for_class(2))
        self.btn_class_two.pack(anchor=tk.CENTER, expand=True)

        self.btn_train = tk.Button(self.window, text="Train Model", width=50, font=("Helvetica", 12),
                                   command=lambda: self.model.train_model(self.counters))
        self.btn_train.pack(anchor=tk.CENTER, expand=True)

        self.btn_predict = tk.Button(self.window, text="Predcit", width=50, font=("Helvetica", 12), command=self.predict)
    ...
    def save_for_class(self, class_num):
        ...
        img.thumbnail((150, 150), PIL.Image.ANTIALIAS)  # Pooling
